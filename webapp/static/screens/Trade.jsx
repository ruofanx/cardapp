...
const { useState: useStateTrade, useEffect: useEffectTrade, useRef: useRefTrade } = React;
...
async function findCandidates({ query, image, identifyCard }) {
  ...
  if (seedName && window.api) {
    const tasks = [];
    if (window.api.searchPokemonTCG) tasks.push(window.api.searchPokemonTCG({ name: seedName }, { pageSize: 20 }));
    if (window.api.searchTCGdex)     tasks.push(window.api.searchTCGdex({ name: seedName }, { pageSize: 20, lang: 'en' }));
...
async function priceCard(card) {
  ...
  if (!price && window.api?.quotePrice) {
    const q = await window.api.quotePrice(card);
...
function TradeScreen({ tweaks, navigate, identifyCard, currentUser, collection, backend, params = {} }) {
  ...
  const [phase, setPhase]           = useStateTrade('idle');   // idle | scanning | picking | loading | offers
  const [query, setQuery]           = useStateTrade('');
  const [candidates, setCandidates] = useStateTrade([]);
  const [loadingCard, setLoadingCard] = useStateTrade(null);
  const [targets, setTargets]       = useStateTrade([]); // [{ card, price, source }, ...] — cards you want
  const [offers, setOffers]         = useStateTrade(null);
  const [error, setError]           = useStateTrade(null);
  const [tradeInFactor, setTradeInFactor] = useStateTrade(0.85);
  const [manualOptions, setManualOptions] = useStateTrade([[], []]); // two binder-built offers
  const [pickerFor, setPickerFor]   = useStateTrade(null); // null | 0 | 1 — which option the picker is adding to
  const [addTargetOpen, setAddTargetOpen] = useStateTrade(false);
  const fileInputRef = useRefTrade(null);
...
function TradeOfferRow({ opt, targetPrice, tradeInFactor, cur, tweaks }) {
  const [expanded, setExpanded] = useStateTrade(false);
...
function BinderPickerSheet({ collection, excludeIds, onPick, onClose, cur, tweaks }) {
  const [search, setSearch] = useStateTrade('');
  const [showAll, setShowAll] = useStateTrade(false);
...
function AddTargetSheet({ identifyCard, tweaks, cur, onAdd, onClose }) {
  const [mode, setMode]             = useStateTrade('search'); // search | scanning | picking | pricing
  const [query, setQuery]           = useStateTrade('');
  const [candidates, setCandidates] = useStateTrade([]);
  const [error, setError]           = useStateTrade(null);
  const fileInputRef = useRefTrade(null);
