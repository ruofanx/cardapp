...
function buildSourceLinks({ card, lang, grading, grader, grade }) {
  ...
  let ebayUrl;
  try {
    ebayUrl = api?.buildEbayUrl
      ? api.buildEbayUrl({ ...card, lang }, isGraded ? { grader, grade } : {})
      : null;
...
function DetailScreen({ tweaks, navigate, addToCollection, removeCard, refreshPrice, updateCard, collection, params = {} }) {
  ...
  const [condition, setCondition] = useState(params.card?.condition || 'NM');
  const [lang, setLang] = useState(params.card?.lang || 'EN');
  // Grading state: 'raw' or 'graded'. Initialize from the card so a graded
  // import lands in graded mode without the user toggling.
  const [grading, setGrading] = useState(params.card?.is_graded ? 'graded' : 'raw');
  const [grader, setGrader] = useState(params.card?.grader || 'PSA');
  const [grade, setGrade] = useState(params.card?.grade != null ? params.card.grade : 10);
  ...
  const c = live || params.card || (CARDS && CARDS[0]) || {};
  const _base = api?.state?.base || 'http://localhost:8000';
  const serverPhotoUrl = c.photo_path ? `${_base}${c.photo_path}` : null;
  const allPhotos = serverPhotoUrl
    ? [{ url: serverPhotoUrl, ts: 'server', isServer: true }, ...userPhotosList]
    : userPhotosList;
  ...
  React.useEffect(() => {
    ...
    if (!api?.quotePrice || !c?.id) return;
    let cancelled = false;
    const t = setTimeout(async () => {
      setQuoting(true);
      try {
        const res = await api.quotePrice(priceQuery());
        if (cancelled) return;
    ...
    try {
      for (const f of files) {
        ...
        try {
          ...
          fd.append('photo', f);
          const res = await fetch(`${api?.state?.base || 'http://localhost:8000'}/api/cards/${c.id}/photo`, {
            method: 'POST', body: fd,
          });
          if (res.ok) {
            serverOk = true;
        ...
        if (!serverOk) {
          try { await userPhotos.add(c.id, f); }
  ...
  const handleRemovePhoto = async (idx) => {
    ...
    if (photo?.isServer) {
      ...
      userPhotos.remove(c.id, serverPhotoUrl ? idx - 1 : idx);
    }
    setLightboxIdx(null);
  };
  ...
  const [resettingArt, setResettingArt] = useState(false);
        ...
        <div style={{ padding: '20px 16px 16px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="row gap-2" style={{ alignItems: 'flex-start', flexWrap: 'wrap' }}>
              ...
              {api?.isSealedProduct(c) && <ProductTypeBadge type={c.product_type} />}
            </div>
          ...
          {!api?.isSealedProduct(c) && (
            <>
              <div className="row gap-2" style={{ marginBottom: 8 }}>
                <div style={{ flex: 1 }}>
                  <SegmentedControl
                    options={[{ value: 'EN', label: 'EN' }, { value: 'JP', label: 'JP' }, { value: 'CH', label: 'CH' }]}
...
function OverviewTab({ card, cur, points, view, refreshing, onRefresh }) {
  ...
  React.useEffect(() => {
    if (!card?.name || !api?.getSoldListings) { setSoldListings([]); return; }
    let cancelled = false;
    setSoldListings(null);
    (async () => {
      try {
        const isGraded = view?.grading === 'graded';
        const res = await api.getSoldListings(card, {
          lang:      view?.lang,
...
function SalesTab({ cur, card, points, view }) {
  ...
  React.useEffect(() => {
    if (!card?.name || !api?.getSoldListings) {
      setData({ sales: [] });
      return;
    }
    let cancelled = false;
...
function VariantsTab({ card, cur, tweaks, navigate }) {
  const [variants, setVariants] = useState(null); // null = loading, [] = none
  const [error, setError] = useState(null);

  React.useEffect(() => {
    if (!card?.name || !api?.searchPokemonTCG) {
      setVariants([]);
      return;
    }
    let cancelled = false;
    (async () => {
      ...
      setError(null);
      try {
        const hits = await api.searchPokemonTCG(
          { name: card.name },
          { pageSize: 16 }
        );
        if (cancelled) return;
