...
export function TweakText({ label, value, placeholder, onChange }) {
  return <TweakRow label={label}><input className="twk-field" type="text" value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} /></TweakRow>
}
...
export function TweakNumber({ label, value, min, max, step = 1, unit = '', onChange }) {
  ...
  const onScrubStart = (e) => {
    e.preventDefault(); startRef.current = { x: e.clientX, val: value }
    const decimals = (String(step).split('.')[1] || '').length
    const move = (ev) => { const dx = ev.clientX - startRef.current.x; const raw = startRef.current.val + dx * step; onChange(clamp(Number((Math.round(raw / step) * step).toFixed(decimals)))) }
